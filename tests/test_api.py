...
class TestPileBulkCreate:
    ...
    def test_import_csv_dry_run_success(self, api_client, project):
        ...
        csv_content = (
            ...
            f"P-301,BORED,750,18.0,17.5,{project.id}\n"
        )
    ...
    def test_import_csv_dry_run_with_errors(self, api_client, project):
        ...
        csv_content = (
            ...
            f"P-401,BORED,INVALID,18.0,17.5,{project.id}\n"
        )
        file = io.BytesIO(csv_content.encode("utf-8"))
        file.name = "piles.csv"
        response = api_client.post(
    ...
    def test_import_csv_success(self, api_client, project):
        ...
        csv_content = (
            ...
            f"P-101,BORED,750,18.0,17.5,{project.id}\n"
        )
        file = io.BytesIO(csv_content.encode("utf-8"))
        file.name = "piles.csv"
        response = api_client.post(
    ...
    def test_import_csv_row_errors(self, api_client, project):
        ...
        csv_content = (
            ...
            f"P-201,BORED,INVALID,18.0,17.5,{project.id}\n"
        )
        file = io.BytesIO(csv_content.encode("utf-8"))
        file.name = "piles.csv"
        response = api_client.post(
        ...
        assert len(data["errors"]) == 1
        assert data["errors"][0]["row"] == 3
        # Shou mention the invalid field
        assert "diameter_mm" in str(data["errors"][0]["errors"])
...
class TestProjectEndpoints:
    def test_boq_csv_export(self, api_client, project):
        ...
        assert ("Pile No,Pile Type,Diameter (mm),"
                "Design Length (m),Actual Length (m),Steel (kg),Steel (tons),"
                "Concrete (m3),Main Bars (kg),Helix (kg),Stiffeners (kg)" in content)
        # Check pile data row
        assert "P-001" in content
