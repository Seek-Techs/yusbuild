...
export function ProjectBoqPage() {
  ...
  const totalConcrete = boqLines.reduce((total, line) => total + line.concreteM3, 0);
              ...
              <TableBody>
                {boqLines.map((line) => (
                  <TableRow key={line.pileType}>
                    <TableCell className="font-medium">{line.pileType}</TableCell>
                    <TableCell className="text-right">{line.count}</TableCell>
                    <TableCell className="text-right">{formatKg(line.steelKg)}</TableCell>
                    <TableCell className="text-right">{formatM3(line.concreteM3)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
              <TableFooter>
                <TableRow>
                  ...
                  <TableCell className="text-right">{formatKg(totalSteel)}</TableCell>
                  <TableCell className="text-right">{formatM3(totalConcrete)}</TableCell>
                </TableRow>
              </TableFooter>
            </Table>
          </CardContent>
