...
import { projects, type ProjectPrototype } from "@/features/prototype/data";

const AXIS_COLOR = "hsl(var(--muted-foreground))";
const GRID_COLOR = "hsl(var(--border))";
const STEEL_COLOR = "hsl(var(--primary))";
const CONCRETE_COLOR = "hsl(var(--accent))";
    ...
    <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
      <Card className="rounded-md">
        <CardHeader className="p-4">
          <CardTitle className="text-base">Steel &amp; concrete by project</CardTitle>
        </CardHeader>
        <CardContent className="h-72 p-4 pt-0">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={quantityData} margin={{ left: 4, right: 4 }}>
              ...
              />
              <Legend
                formatter={(value) => (value === "steelKg" ? "Steel (kg)" : "Concrete (m3)")}
                wrapperStyle={{ fontSize: 12, color: AXIS_COLOR }}
              />
              <Bar yAxisId="steel" dataKey="steelKg" fill={STEEL_COLOR} radius={[4, 4, 0, 0]} />
              <Bar
                yAxisId="concrete"
                dataKey="concreteM3"
                fill={CONCRETE_COLOR}
            ...
            <PieChart>
              <Tooltip
                ...
                formatter={(value, _key, entry) => [
                  value,
                  STATUS_LABELS[
                    (entry.payload as { status: ProjectPrototype["status"] }).status
                  ],
                ]}
              />
              <Legend
                formatter={(_value, entry) => {
                  const payload = entry.payload as unknown as { status: ProjectPrototype["status"] };
